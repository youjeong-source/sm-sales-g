...
export default async function handler(req, res) {
  ...
  // KV 설정 안 됨 → 명확한 에러 반환 (프론트가 fallback)
  if (!KV_URL || !KV_TOKEN) {
    return res.status(503).json({
      error: 'KV_NOT_CONFIGURED',
      message: 'Vercel KV 환경변수가 설정되지 않았습니다. Vercel 대시보드 → Storage → KV 데이터베이스 생성 후 프로젝트에 연결하세요.'
    });
  }
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Upstash Redis REST: POST {KV_URL}/set/{key} with value as raw body
      const r = await fetch(`${KV_URL}/set/${KEY}`, {
        ...
        body: JSON.stringify(payload)  // double-encode: outer JSON wraps the inner JSON string
      });
      if (!r.ok) {
        return res.status(500).json({ error: 'KV_SET_FAILED', status: r.status });
      }
